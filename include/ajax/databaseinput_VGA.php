 <?php
         ...
         <label class="nameprotocol<?php echo $s;?>" style="">Protocol</label>
          ...
          <div class="col-md-2">
          <input type="button" value="update" class="update update<?php echo $s; ?>" isid="<?php echo $s; ?>" name="update" style="background: #CF0A2C;color: white;    margin-top: 18px;" > 
          </div>
          </div>
          </div>
          ...
          <select style="width: 110px;height: 20px;" id="devicename<?php echo $s; ?>" class="device" isid="<?php echo $s; ?>">
            ...
            <?php 
              $viw=mysql_query("SELECT TypeID from inputdevices where VGA=1 group by TypeID");
              while($sow=mysql_fetch_assoc($viw)) {
              echo '<option value="'.$sow['TypeID']. '">'.$sow[ 'TypeID']. '</option>';
              } 
              ?> 
         ...
         <label class="nameprotocol<?php echo $s;?>" style="display: none;">Protocol</label>
          ...
          <div class="col-md-2">
          <input type="button" value="save" class="button button<?php echo $s; ?>" isid="<?php echo $s; ?>" name="save" style="background: #CF0A2C;color: white;    margin-top: 18px; display: none;" > 
          </div>
          <div class="col-md-2">
          <input type="button" value="update" class="update update<?php echo $s; ?>" isid="<?php echo $s; ?>" name="update" style="background: #CF0A2C;color: white; display: none;   margin-top: 18px;" > 
          </div>
          </div>
          </div>
